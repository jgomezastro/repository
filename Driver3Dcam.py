...
from openni.openni2 import initialize
from openni.openni2 import Device
from openni.openni2 import unload
from openni._openni2 import ONI_DEVICE_PROPERTY_SERIAL_NUMBER
...
#    @profile
    def __init__(self, fps_window=30):
        ...
        if platform.system() == "Windows":
            initialize("C:/Program Files/OpenNI2/Redist") #specify path for Redist
        else:
            initialize()        #######################################  adds 45.5 MiB 
        self.dev = Device.open_any()             #######################################  adds 6.7 MiB
        self.serial_number = (self.dev.get_property(ONI_DEVICE_PROPERTY_SERIAL_NUMBER, (c_char * 100)).value).decode('utf-8')
...
#    @profile
    def ir_frame_callback(self, stream):
        ...
        self.ir_frame = asarray(ir_frame).reshape((self.height, self.width))
...
#    @profile
    def depth_frame_callback(self, stream):
        ...
        #Converting array into frame and frame update
        self.depth_frame = asarray(buffer_copy).reshape((self.height, self.width))
        self.depth_frame = where(self.depth_frame > self.max_range, self.max_range, self.depth_frame)
